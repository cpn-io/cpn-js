...
export class ProjectDeclarationsComponent implements OnInit, AfterViewInit, DoCheck {
  ...
  ngOnInit() {
    ...
    this.eventService.on(Message.MODEL_RELOAD, () => this.loadProject());
     this.eventService.on(Message.SERVER_INIT_NET_DONE, () => this.dragAndDropHandler());
    ...
    setTimeout( function ()  {
      ...
      function handleDrop(e) {
        ...
        if (dragSrcEl !== this) {
          ...
          searchElemForInsert(self.cpnet.globbox.block, self.cpnet.globbox, resultFrom, idFrom, resultFrom.index  );
          searchElemForInsert(self.cpnet.globbox.block, self.cpnet.globbox, resultTo, idTo, resultTo.index );
          ...
          // }
          resultTo.array.splice(parseInt(resultTo.index, 10), 0, resultFrom.array[resultFrom.index]);
