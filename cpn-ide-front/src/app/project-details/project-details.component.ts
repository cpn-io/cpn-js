...
//   constructor(private eventService: EventService, private settings: Settings, private modelService: ModelService) {
//   }

//   ngOnInit() {
//      Subscribe on project load event
//     is.eventService.on(Message.PROJECT_LOAD, (data) => {
//     this.loadProject(data);
//       this.currentPojectModel = data;
//     });

//     is.eventService.on(Message.OPEN_DECLARATION_BLOCK, (data) => {
//       t tab = this.tabsComponent.tabs.find(e => e.id === data.id);
//        (tab) {
//         t oldSelectedTab = this.tabsComponent.getSelectedTab();
...
//       break;
//       se 'rename':
//         is.changeinExplorerName(data);
//         is.eventService.send(Message.UPDATE_TREE, {
...
//         //cpnet.globbox.block = cpnet.globbox.block.f ilter(e => e.id !== data.id);
...
//     abDblClick(tabid) {
//      (event.target['className'] !== 'tabs__tab active') {
...
//     id: this.settings.getAppSettings()['block'] + (++this.countNewItems)
//   };
...
//   /
//   dItemClick(blockId, elementType, state) {
//     t comBlocks = false;
//      (blockId === elementType) {
...
// //   switch (elementType) {
// //     case 'var':
// //       return {id: this.settings.getAppSettings()[elementType] + (++this.countNewItems), type: {id: this.settings.getAppSettings()[elementType]}, _id: 'ID' + new Date().getTime()};
// //       break;
// //     case 'color':
// //       return {id: this.settings.getAppSettings()[elementType] + (++this.countNewItems), timed: '', name: this.settings.getAppSettings()[elementType], _id: 'ID' + new Date().getTime()};
// //       break;
// //     case 'ml':
// //       return {
...
// //       break;
// //     case 'globref':
// //       return {id: this.settings.getAppSettings()[elementType] + (++this.countNewItems), ml: this.settings.getAppSettings()[elementType], _id: 'ID' + new Date().getTime()}
// //       break;
// //     default:
// //
//   //   }
...
//   leteRow(tabsComp, state) {
//   console.log('DELETE ROW click');
//     t afterDelDec;
//      (this.selectedElemement) {
...
//       is.selectedElemement = undefined;
//       is.eventService.send(Message.UPDATE_TREE, {
//       project: this.currentPojectModel,
//       state: state,
...
//   leteTab(tabsComp, state) {
//     nsole.log('DELETE TAB click, tabsComp = ' + JSON.stringify(tabsComp));
//      (tabsComp) {
...
//     cpnet.globbox.block = cpnet.globbox.block.f ilter(e => e.id !== tabsComp.selectedTab.id);
//   }
//     this.eventService.send(Message.UPDATE_TREE, {project: this.currentPojectModel, state: state});
//   }


//     ctClick(item, tabid, elemT ype) {
//   if (elemType) {
...
//         // --------------------------------------
//           t params = [];
//             (let paramElem of this.paramsTypes) {
//             t paramBlock = {
//             id: paramElem,
//             block: []
//           };
...
//   ndblockById(id, globoxBlock): any {
//     t result;
//       (const block of globoxBlock) {
//     if (block.id === id) {
//       return block;
...
//   archBlocks(currentBlock, params) {
//     is.blocksToArray(currentBlock);
//     r (const block of currentBlock) {
//     this.blocksToArray(block);
//     //dataSets[block.id] = block;
//       is.declarations.push(block);
//     if (block.color) {
...
//   tColorStringValue(color): string {
//   /*
//     t colorStringValue;
...
//         de.name += ' = product ';
//           color.product.id instanceof Array) {
...
//     lockElemeStringValue(item, type) {
//       ch (type) {
//       se 'var':
...
//     t htmlElement: HTMLElement = <HTMLElement>event.target;
//       htmlElement && (htmlElement.nodeName === 'TD' || htmlElement.nodeName === 'TR')) {
//        (htmlElement.offsetParent) {
//         r table: HTMLTableElement = <HTMLTableElement>document.getElementById(htmlElement.offsetParent.id);
//          (table.id) {
//           t parsId = table.id.split('-');
//             parsId[0] === 'declarationPanel') {
//               parsId.length > 3) {
...
//  * @param globref
//    */
//   tFirstCellDataFromMLCode(globref): string {
...
//     t array = str.match('^((local){1}\\s+)*(fun|val|exception){1}\\s+');
//   if (array) {
//     firstCell = array[0].trim();
//     else {
//       ray = str.match('^[a-zA-Z0-9_]+\\s*:');
//     if (array) {
//       firstCell = 'var';
//       else {
//         ray = str.match('^[a-zA-Z0-9_]+\\s*=');
...
//   /
//   angeinExplorerName(data) {
//     t block;
//       ch (data.element) {
//       se 'ml':
//         ock = this.declarations.find(e => e.id === data.target).ml;
...
//       break;
//       se 'var':
//         ock = this.declarations.find(e => e.id === data.target).var;
//       if (block instanceof Array) {
//         block = block.find(e => e._id === data.id);
//       }
...
//       break;
//       se 'color':
//         ock = this.declarations.find(e => e.id === data.target).color;
//       if (block instanceof Array) {
//         block = block.find(e => e._id === data.id);
//       }
...
//       break;
//       se 'globref':
//         ock = this.declarations.find(e => e.id === data.target).globref;
//       if (block instanceof Array) {
//         block = block.find(e => e._id === data.id);
//       }
...
//   itVarTable(tabId, table) {
//   let block = this.declarations.find(elem => elem.id === tabId.replace('-commonTable', '')).var;
//     r rowLength = table.rows.length;
//     r (var i = 0; i < rowLength; i += 1) {
...
//     var input = table.rows[i].cells[1].textContent;
//     console.log(input);
//      his.modelService.parseVariableLayout(input, block[i], 'color');
//      * let words = input.split(/\s+/)
//        rds = words.filter(elem => elem !== '');
//          !this.standartDeclaration.includes(table.rows[i].cells[0].textContent)) {
//           (words.length === 1) {
