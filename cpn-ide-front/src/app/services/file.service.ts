...
export class FileService {
  ...
  constructor(private eventService: EventService, private electronService: ElectronService, private accessCpnService: AccessCpnService) {
  }
    ...
    if (this.electronService.isElectronApp) {
      this.electronService.remote.dialog.showSaveDialog({
        ...
        if (newFileName) {
          ...
          console.error(this.constructor.name, 'onSaveProject(), ERROR filename, f = ', newFileName);
        }
      });
    } else {
      const blob = new Blob([charArray], { type: 'application/xml; charset=iso-8859-1' });
