...
export function fitToView(diagram) {
  const canvas = diagram.get("canvas");

  const vb = getViewbox(diagram);
  if (vb) {
    canvas.resized();
    canvas.viewbox(vb);
    // canvas.zoom(0.7);
  ...
  return new Promise((resolve, reject) => {
    ...
    function render(cpnPageElement) {
      ...
      if (!cpnPageElement.pageattr)
        throw new Error(
          translate(
            "CPN page element is not a CPN page object (pageattr is undefined)"
          )
        );
